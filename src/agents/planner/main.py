...
# LLM Client (provider-agnostic: supports Anthropic or OpenAI)
try:
...
async def extract_locations_with_llm(query: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    if not llm:
        log.warning("No LLM client available, falling back to basic parsing")
...
async def a2a_message(message: A2AMessage):
    ...
    Uses LLM to extract locations from natural language.
    """
